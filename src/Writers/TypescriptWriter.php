...
class TypescriptWriter implements WriterContract
{
    ...
    public function write(GenerationResult $result, WriterConfig $config): WriterOutput
    {
        ...
        foreach ($this->renderImportStatements($result->imports) as $statement) {
            $lines[] = $statement;
        }
    ...
    private function writePerModelFiles(
        ...
        foreach ($result->models as $modelResult) {
            ...
            foreach ($this->renderImportStatements($modelResult->imports) as $statement) {
                $lines[] = $statement;
            }

            if (! empty($modelResult->imports)) {
                $lines[] = '';
            }
     ...
     * @return list<string>
     */
    private function renderImportStatements(array $imports): array
    {
        return collect($imports)
            ->filter(fn (array $entry) => isset($entry['import'], $entry['type']) && $entry['import'] !== '' && $entry['type'] !== '')
            ->map(fn (array $entry) => "import type { {$entry['type']} } from '{$entry['import']}';")
            ->unique()
            ->sort()
            ->values()
            ->all();
